...
export default function ProjectCarousel2() {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await axios.get(
          `https://api.unsplash.com/search/photos?query=residential-exterior&orientation=landscape&client_id=${ACCESS_KEY}`,
        );
    ...
    const fetchProject = async () => {
      try {
        const response = await axios.get('https://jawabu-interiors-backend.onrender.com/projects/6');
